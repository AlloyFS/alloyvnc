...
use alloyvnc_screen::{Capture, NullInput};
...
async fn serve(args: ServeArgs) -> Result<()> {
    ...
    let capture: Box<dyn Capture> = match args.backend {
        Backend::Synth => Box::new(Synth::new(width, height, Pace::Fps(args.fps))),
    };
    let (width, height) = capture.size();
    let shared = Shared::new(args.name, width, height, Box::new(NullInput));
